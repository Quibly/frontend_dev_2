...
export default class QuakesController {
  ...
  async init() {
    ...
    console.log('Quakes Controller Init Successful.')
  }
  async initPos() {
  ...
  async getQuakesByRadius(radius = 100) {
    ...
    this.parentElement.addEventListener('click', e => {
        if (e.target.childNodes.length == 1) {
          this.getQuakeDetails(e.target.dataset.id, e.target);
        } else {
  ...
  async getQuakeDetails(quakeId, element) {
    ...
    const quakeDetails = await this.quakes.getQuakeById(quakeId);
    this.quakesView.renderQuake(quakeDetails, element);
  }
}